...
<body>
                    ...
                    <div class="chat">
                            ...
                            <div class="row">
                                <div class="col-lg-6">
                                    ...
                                    <div class="chat-about">
                                        <h6 class="m-b-0" id="user-name">{{ $user->role === 'admin' ? $chatLatest->nama : 'Admin' }}</h6>
                                        <small>Online</small>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="chat-message clearfix">
                            <div class="input-group mb-0">
                                <div class="input-group-prepend">
                                    <span class="input-group-text" id="send-message">
    ...
    <div id="admin_name" data-admin-nama="{{ $user->nama }}"></div>
    ...
    <script>
        $(document).ready(function() {
            ...
            var admin_id = $('#admin_id').data('admin-id')
            var admin_name = $('#admin_name').data('admin-nama')
            ...
            if (role === 'admin') {
                ...
                user_id = Number(document.querySelector('.chat-list > li.active').getAttribute('data-user-id'))
                setTimeout(() => {
                    setInterval(() => {
                        $.get(`/ajax/chats/${user_id}`)
                            .done(({ data: { chats } }) => {
                                ...
                                if (chats.length > 0) {
                                    chats.forEach((chat, index) => {
                                        if (chat.from_me && admin_id !== chat.admin_id) {
                                            $('.chat-history ul').append(`
                                                <li class="clearfix">
                                                    <div class="message-data">
                                                        <span class="message-data-time">${chat.created_at_formatted_human}</span>
                                            ...
                                            $('.chat-history ul').append(`
                                                <li class="clearfix">
                                                    <div class="message-data text-right">
                                                        <span class="message-data-time">${chat.created_at_formatted_human}</span>
                                                        <span class="message-data-time-datetime">${chat.created_at_formatted}</span>
                                                        <img src="https://ui-avatars.com/api/?name=${(admin_name).replace(' ', '+')}" alt="avatar">
                                                    </div>
                                                    <div class="message other-message float-right">${chat.message}</div>
                                                    <span class="message-mark-status float-right">${index+1 === chats.length ? (chat.mark_read ? 'Dibaca' : '') : ''}</span>
                                                </li>
                        ...
                        // console.log('user_id inside admin', user_id)
                    }, 2_000);
                }, 3_000);
            }
            ...
            $('ul.chat-list li').click(function(e) {                
                ...
                if (role === 'admin') {
                    ...
                    document.querySelector(`.chat-list > li#chat-user-${user_id}`).classList.add('active')

                    
                    $.ajax({
                        url: `/ajax/chats/${user_id}`,
                        type: 'PUT',
                ...
                if (!message) {
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            // $('#user-name').text(user.nama)
                            console.info('PESAN TERKIRIM', response)
                        }
                    })
                    // console.log('CLICK SEEND MESSAGE', user_id, admin_id, from_me, message)
                }
            })
        })
    </script>
