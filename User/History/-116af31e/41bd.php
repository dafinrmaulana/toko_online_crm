...
class ChatController extends Controller
{
    public function index()
    {
        ...
        if ($user->role === 'admin') {
            ...
            $chatLatest->chats->each(function($chat) {
                ...
                $chat['created_at_formatted'] = Carbon::parse($chat->created_at)->format('d M H.i');
    ...
    public function update(Request $request, Chat $chat) {
        if (@$chat) {
            $chat->mark_read = $request->mark_read ?? $chat->mark_read;
            $chat->save();
    ...
    public function chats($user_id) {
        $chats = Chat::where('admin_id', $user_id)->get();
        // $users = Admin::where('role', 'admin')->first();

        // $chatLatest = Admin::where('id', $user_id)->with('chats')->first();
